    ...
    return(
        <div className="image-carousel">
                ...
                {restData.map(project =>
                    <img key={project.id}
                     srcSet={`${project.acf.images[index].sizes['large']} 1024w,
                              ${project.acf.images[index].sizes['2048x2048']} 2048w`}
                     sizes="(max-width: 736px) 1024px, 2048px"
                     src={project.acf.images[index].sizes['2048x2048']}
                     alt={project.acf.images[index].alt} />
                    )}
        </div>
    );
}
